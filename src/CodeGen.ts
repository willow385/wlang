...
import { isPointer, isMut, isModuleTypeSound } from "./TypeChecker";
...
export function compile(moduleAst: Module): string {
  if (!isModuleTypeSound(moduleAst)) {
    throw new Error(
      "The program violates the rules of the Wlang type system and therefore could not be compiled."
    );
  }
...
`#include <stdint.h>
#include <stddef.h>
// C generated by ${wlangCompilerVersion} of the Wlang compiler
`;
